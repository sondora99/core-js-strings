...
function repeatString(str, times) {
  const newStr = times >= 0 ? str.repeat(times) : '';
  return newStr;
}
...
function removeFirstOccurrences(str, value) {
  ...
  const result =
    temp !== -1
      ? str.substring(0, temp) + str.substring(temp + value.length)
      : str;
  return result;
}
...
function isPalindrome(str) {
  ...
  return (
    cleanedStr.toLowerCase().replaceAll(' ', '') ===
    cleanedStr.replaceAll(' ', '').split('').reverse().join('').toLowerCase()
  );
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (letter === ' ') {
      ...
      newStr += alphabetUp[(alphabetUp.indexOf(letter) + 13) % 26];
    } else if (letter === letter.toLowerCase()) {
      newStr += alphabetLow[(alphabetLow.indexOf(letter) + 13) % 26];
    }
  }
  return newStr;
}
